	...
	return (
		<div className="h-full flex flex-col justify-between">
			<div className="flex flex-col gap-5 font-sans">
				...
				{/* Panel specs overview */}
				{panelSpec && (
					<div className="bg-white/5 border border-white/5 rounded-2xl p-4 flex flex-col gap-2.5 text-xs text-neutral-350">
						<div className="flex items-center gap-1.5 border-b border-white/5 pb-2 text-white font-bold text-[10px] uppercase tracking-wider">
							<HardDrive className="w-3.5 h-3.5 text-teal-400" />
							<span>Active Hardware Spec</span>
						</div>
						<div className="flex justify-between">
							<span>Module Model:</span>
							<span className="font-bold text-white truncate max-w-[160px]">{panelSpec.brand} {panelSpec.name}</span>
						</div>
						<div className="flex justify-between">
							<span>Rating & Dimensions:</span>
							<span className="font-bold text-white text-[11px]">
								{panelSpec.rating}Wp ({((panelSpec.length || 2279) / 1000).toFixed(2)}m × {((panelSpec.width || 1134) / 1000).toFixed(2)}m)
							</span>
						</div>
					</div>
				)}
					...
					{!isPlacingGroup ? (
						...
						<div className="flex flex-col gap-2">
							...
							<button
								...
								className="w-full py-2.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-450 text-xs font-bold rounded-xl border border-rose-500/15 transition-all cursor-pointer"
							>
				...
				{selectedGroup ? (
					<div className="bg-white/5 border border-white/5 rounded-2xl p-5 flex flex-col gap-4">
						...
						<button
							...
							className="w-full mt-2 py-2 bg-rose-500/10 hover:bg-rose-500/20 text-rose-450 text-xs font-bold rounded-xl border border-rose-500/15 transition-all flex items-center justify-center gap-1.5 cursor-pointer"
						>
